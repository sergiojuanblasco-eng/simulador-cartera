...
export default function Login({ go, session, mode }) {
      ...
      <div style={{ background: V.card, border: `1.5px solid ${V.border}`, borderRadius: 20, padding: "40px 40px 36px", width: "100%", maxWidth: 420, boxShadow: "0 8px 40px rgba(0,0,0,0.07)" }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input type="password" placeholder="••••••••" value={pass} onChange={e => setPass(e.target.value)} required style={inp} onKeyDown={e => e.key === "Enter" && handleSubmit(e)} />
          </div>
        ...
        <div style={{ textAlign: "center", marginTop: 20, fontSize: 13, color: V.muted }}>
          {isSignup
            ? <>¿Ya tienes cuenta? <span style={{ color: V.green, fontWeight: 700, cursor: "pointer" }} onClick={() => go("/login")}>Entrar</span></>
            : <>¿No tienes cuenta? <span style={{ color: V.green, fontWeight: 700, cursor: "pointer" }} onClick={() => go("/registro")}>Crear cuenta gratis</span></>
          }
        </div>

        {isSignup && <div style={{ textAlign: "center", marginTop: 20, fontSize: 11, color: V.muted, lineHeight: 1.6 }}>
